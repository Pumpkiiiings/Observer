...
public class TextComponent implements ObserverComponent {
    ...
    private static Object getMatricesSafe(GuiGraphicsExtractor context) {
        if (!matrixSearched) {
            matrixSearched = true;
    ...
    private static void renderText(GuiGraphicsExtractor context, Minecraft client, Component text, int x, int y) {
        ...
        try {
            if (cachedDrawMethod != null) {
                ...
                context.text(client.font, text, x, y, 0xFFFFFFFF);
            }
        } catch (Exception e) {
